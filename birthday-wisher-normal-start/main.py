...
# HINT 3: Use the replace() method to replace [NAME] with the actual name. https://www.w3schools.com/python/ref_string_replace.asp
    ...
    if choice == 1:
        ...
        birthday_letter = contents.replace("[NAME]", birthdays_dict[today]['name'])
        with open("letter_templates/letter_1.txt", mode="w") as letter:
            letter.write(birthday_letter)
        with open("letter_templates/letter_1.txt", mode="r") as letter:
            final_contents = letter.read()

    elif choice == 2:
        with open("letter_templates/letter_2.txt") as letter:
            contents = letter.read()
        birthday_letter = contents.replace("[NAME]", birthdays_dict[today]['name'])
        with open("letter_templates/letter_2.txt", mode="w") as letter:
            letter.write(birthday_letter)
        ...
        with open("letter_templates/letter_3.txt") as letter:
            contents = letter.read()
        birthday_letter = contents.replace("[NAME]", birthdays_dict[today]['name'])
        with open("letter_templates/letter_3.txt", mode="w") as letter:
            letter.write(birthday_letter)
